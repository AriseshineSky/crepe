...
async function parseYisucangRow(row, HEADER) {
	...
	let boxCount = parseInt(row[HEADER.indexOf("入库数量")]) || 30;
	let unitsPerBox = parseInt(row[HEADER.indexOf("个数/箱")]) || 30;
	let date = row[HEADER.indexOf("入库时间")];
	let logisticsTrackingNumber = row[HEADER.indexOf("物流追踪单号")];
	let quantity = boxCount * unitsPerBox;
	return {
		quantity,
		number,
		orderId,
...
async function syncYisucangInbounds() {
	...
	[...reciveds, ...inbounds].forEach((inbound) => {
