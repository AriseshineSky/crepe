...
async function checkAllProducts(message) {
	...
	await sheetApi.setHeader(
		singleSheet,
		["PM", "ASIN", "country", "7 days average sales", "account"],
		1,
	);
...
async function checkProductByListing(
	product,
	index,
	total,
	singleSheet,
	...
	for (let country of product.counties) {
		await checkProductByCountry(product, listings, country, singleSheet, shortageSheet, message);
	}
	await checkProductInventory(product, listings, inventorySheet, message);
}

async function inventoryShortageRecord(
	asin,
	country,
	data,
	totalDays,
	...
	var content = `${pm}\t${asin}\t${country}\t${data.availableQuantity}\t${
		data.reservedFCTransfer
	}\t${data.inboundShipped}\t${data.sales.toFixed(2)}\t${availableDays.toFixed(
		2,
	)}\t${totalDays.toFixed(2)}\n`;
...
async function singleInventoryRecord(asin, country, account, data, sheet, message) {
	var pm = await getPm(asin, country);
...
async function checkProductStockShortage(stock, sales) {
	let days = stock / sales;
	if (days < 30) {
	}
}
...
async function checkProductInventory(product, listings, inventorySheet, message) {
	...
	if (
		moment(orderDues.seaExpress).diff(moment(new Date()), "days") < 7 ||
		(product.stock + product.plwhs) / sales < 30
	) {
		sendMessage(
			product,
...
async function checkProductByCountry(
	product,
	listings,
	country,
	singleSheet,
	...
	if (data.sales < 1) {
		...
		if (account) {
			await singleInventoryRecord(product.asin, country, account, data, singleSheet, message);
		}
	}
}
