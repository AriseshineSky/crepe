...
async function checkProducts(listings) {
  ...
  for (var i = 0; i < products.length; i++) {
    if (listings[products[i].asin]) {
      await checkProduct(listings[products[i].asin], products[i], i + 1, products.length, singleSheet, shortageSheet, inventorySheet);
    }
  }
}
...
function inventoryCheckRecord(asin, type, quantity, producingStatus, inventorySheet) {
  getPm(asin, 'US').then(async function(pm) {
    var content = `${pm}\t${asin}\t${quantity.quantity}\t${producingStatus.orderDues['airExpress']}\t${producingStatus.orderDues['airDelivery']}\t${producingStatus.orderDues['seaExpress']}\t${producingStatus.orderDues['sea']}\t${massage[type]}\n`;
...
async function checkProductInventory(product, listings, inventorySheet) {
  ...
  await Product.syncFreight(product, 2);
  ...
  if (product.asin == 'B09BXWFYWC') {
    logger.debug('product1', product, quantity, totalInventory, sales);
  }
  if (quantity.boxes <= 0) {
    ...
    if (product.asin == 'B09BXWFYWC') {
      logger.debug('product2', product);
    }
    return quantity;
  }
  logger.debug('product3', product);
  await Product.updateProduct(product, {orderQuantity: quantity.quantity});
  ...
  logger.debug(product.asin, orderDues);
  var producingStatus = await Product.checkProducingsCreated(orderDues, quantity, product, sales);
  await Product.updateOrderDues(product, producingStatus);
  await Product.save(product);
  logger.debug('producingStatus', producingStatus);
  logger.debug('product', product);
  if (!producingStatus.placed) {
    if (moment(producingStatus.orderDues.airExpress).diff(moment(new Date()), 'days') < 5) {
      inventoryCheckRecord(product.asin, 'airExpress', quantity, producingStatus, inventorySheet);
    } else if (moment(producingStatus.orderDues.seaExpress).diff(moment(new Date()), 'days') < 5 && moment(producingStatus.orderDues.seaExpress).diff(moment(new Date()), 'days') >= 0) {
      inventoryCheckRecord(product.asin, 'seaExpress', quantity, producingStatus, inventorySheet);
    } else if (moment(producingStatus.orderDues.sea).diff(moment(new Date()), 'days') < 5 && moment(producingStatus.orderDues.sea).diff(moment(new Date()), 'days') >= 0) {
      inventoryCheckRecord(product.asin, 'sea', quantity, producingStatus, inventorySheet);
    }
  }
}
...
var checkProduct = async function(listings, product, index, total, singleSheet, shortageSheet, inventorySheet) {
