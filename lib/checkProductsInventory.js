...
async function checkProductInventory(product, listings, inventorySheet) {
  var fbaInventorySales = await Product.prepareFbaInventoryAndSalesByCountryV2(product.asin, product, listings);
...
async function getListingAccountsCountByCountry(listings, country) {
  var count = 0;
...
async function checkProductSingleInventory(data, listings, country) {
  if ((country.toUpperCase() === 'US' && (data.sales > 30)) || (country.toUpperCase() === 'CA' && (data.sales > 15))) {
    var accountCounts = await getListingAccountsCountByCountry(listings, country);
...
async function checkProductByCountry(product, listings, country, singleSheet, shortageSheet) {
  ...
  if (data.sales < 1) {
    ...
    var account = await checkProductSingleInventory(data, listings, country);
    if (account) {
