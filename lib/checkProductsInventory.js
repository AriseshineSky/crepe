...
function inventoryShortageRecord(asin, country, data, totalDays, availableDays) {
  getPm(asin, country).then(function(pm) {
    ...
    writeToFile(`${process.cwd()}/data/shortage.csv`, content);
  }, function(error) {
    console.log(error);
  });
}
function singleInventoryRecord(asin, country, account, data) {
  getPm(asin, country).then(function(pm) {
    ...
    writeToFile(`${process.cwd()}/data/single.csv`, content);
  }, function(error) {
    console.log(error);
  })
}
...
async function checkProductByCountry(product, listings, country) {
  ...
  if (data.sales < 1) {
    ...
    if (status.shortage) {
      inventoryShortageRecord(product.asin, country, data, status.totalDays, status.availableDays);
    }
