...
async function sendInventoryMessage(asin, country, data, totalDays, availableDays, sheet) {
  var pm = await getPm(asin, country);
  var content = `${pm.name}\t你的产品${asin}\t${country}\t 亚马逊库存只能再卖${totalDays.toFixed(2)}天，请尽快发货\n`;        
...
async function sendAccountMessage(asin, country, account) {
  getPm(asin, country).then(async function(pm) {
    var content = `${pm.name}\t你的产品${asin}\t只有一个账号${account}，请尽快选择备用账号发货。`;       
    bot.sendReminderMessage(pm.chat_id, content);
  }, function(error) {
    console.log(error);
  })
...
async function sendMessage(product, fbaInventory, ps, quantity, orderDues) {
  var pm = null;
  ...
  if (pm && pm.chat_id) {
    var content = `${pm.name}\t你的产品${product.asin}\t<strong color="#FF0000">需要采购${quantity.quantity}</strong>\t
...
async function checkProductInventory(product, listings, inventorySheet) {
  ...
  if (moment(orderDues.seaExpress).diff(moment(new Date()), 'days') < 7) {
    sendMessage(product, fbaInventorySales.inventory, sales.minAvgSales, quantity, orderDues);
  }
}
