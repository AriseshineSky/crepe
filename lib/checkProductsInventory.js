...
function inventoryCheckRecord(asin, country, type, quantity, producingStatus) {
  console.log(asin, type, producingStatus.orderDues[type]);
  getPm(asin).then(function(pm) {
    var content = `${pm}\t${asin}\t${country}\t${type}\t${quantity.quantity}\t${producingStatus.orderDues[type]}\t${massage[type]}\n`;
...
async function checkProductByCountry(product, listings, country) {
  ...
  if (data.sales < 1) {
    ...
    if (Number(fbaInventorySales.sales) < 3) {
      console.log(fbaInventorySales.sales);
      return;
    }
    ...
    if (product.asin === 'B0B8F27R5H') {
      logger.info(product.inboundShippeds)
      logger.info(product.producings)
      logger.info('syncFreight before')
    }
    await Product.syncFreight(product);
    console.log(product.asin, 'syncFreight done');
    if (product.asin === 'B0B8F27R5H') {
      logger.info('syncFreight dnoe')
      logger.info(product.inboundShippeds)
      logger.info(product.producings)
    }
    ...
    if (product.producings) {
      console.log('producings', product.producings.length);
    ...
    if (!producingStatus.placed) {
      if (moment(producingStatus.orderDues.airExpress).isBefore(moment.now())) {
        inventoryCheckRecord(product.asin, country, 'airExpress', quantity, producingStatus);
      } else if (moment(producingStatus.orderDues.seaExpress).isBefore(moment.now())) {
        inventoryCheckRecord(product.asin, country, 'seaExpress', quantity, producingStatus);
      } else if (moment(producingStatus.orderDues.sea).diff(moment(new Date()), 'days') < 7) {
        inventoryCheckRecord(product.asin, country, 'sea', quantity, producingStatus);
      }
    }
  }
}
