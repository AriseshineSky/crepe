var purchaseProcureApi = require("../api/gerpgo/purchaseProcure");
...
async function getPurchaseProcures(token, salt) {
	return new Promise(async (resolve, reject) => {
		var url = `${gerpgo_api_prefix}` + `${open_apis.purchaseProcure}`;
		var i = 1;
		var state = "begin";
		while (state !== "end") {
			// while (i < 2) {
		...
		setTimeout(async function () {
			...
			if (purchaseProcures.data.rows) {
				for (let purchaseProcure of purchaseProcures.data.rows) {
					...
					await Purchase.findOrCreate(purchase);
				}
				resolve("pending");
			} else {
				resolve("end");
...
module.exports.syncPurchaseOrders = async function () {
	token.getToken3(`${gerpgo_api_prefix}` + `${open_apis.token}`).then(
		async function (token) {
			await getPurchaseProcures(token, "6419b1fbe4b07fc573e5ec7e");
		},
		function (error) {
			console.log(error);
		},
	...
	setTimeout(async function () {
		token.getToken(`${gerpgo_api_prefix}` + `${open_apis.token}`).then(
			async function (token) {
				await getPurchaseProcures(token, "62daace5e4b073604b7e0b27");
			},
			function (error) {
				console.log(error);
			},
	...
	setTimeout(async function () {
		token.getToken2(`${gerpgo_api_prefix}` + `${open_apis.token}`).then(
			async function (token) {
				await getPurchaseProcures(token, "6399d977e4b086e9ac8877e0");
			},
			function (error) {
				console.log(error);
			},
