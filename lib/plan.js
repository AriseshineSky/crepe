...
var getStockByProduct = require('./getStockByProduct')
var Product = require('../proxy').Product;

async function prepareStock(asin) {
  var stock = await getStockByProduct(asin);
  if (stock.inventory) {
...
async function checkInventory(freightPlan, freight, product, totalInventory, sales, inboundShippeds) {
  ...
  for (var inbound of inboundShippeds) {
    ...
    if (units + totalInventory - period * sales.minAvgSales < Number(product.minInventory)) {
      return false;
    }  
    data[period] = {
  ...
  for (var type in freightPlan) {
    ...
    if (quantity + totalInventory - (freight[type].period + product.cycle) * sales.minAvgSales < Number(product.minInventory)) {
      return false;
    }  
