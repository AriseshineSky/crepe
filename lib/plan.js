...
module.exports = async function(asin) {
  ...
  var sales = {
    ...
    maxAvgSales: PRODUCTS[asin].maxAvgSales
  };
...
async function calculatePlanAmounts(freightPlan, freight, product) {
  ...
  freightPlan.totalAmount = amount.toFixed(2);
  return freightPlan;
}
async function checkInventory(freightPlan, freight, product, totalInventory, sales, inboundShippeds) {
  ...
  for (var type in freightPlan) {
    ...
    if (freightPlan['sea']['boxes'] == 51) {
      ...
      console.log(freight[type].period + product.cycle);
      console.log(sales.minAvgSales);
      console.log(await totalUnits(freightPlan[type].boxes, product.unitsPerBox));
      console.log(freight[type].period + product.cycle);
      console.log(data[freight[type].period + product.cycle]);
    }
